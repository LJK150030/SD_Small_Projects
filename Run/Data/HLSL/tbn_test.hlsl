...
v2f_t VertexFunction(vs_input_t input)
{
   ...
   float4 world_color  = float4(	(world_norm.x + 1.0f) * 0.5f, 
						(world_norm.y + 1.0f) * 0.5f,
						(world_norm.z + 1.0f) * 0.5f,
						1.0f );
						
						
	v2f.color = world_color;
	v2f.uv = input.uv; 
	v2f.normal = world_norm;
	v2f.tangent = world_tangent;
...
{
	float4 surface_color = tNormal.Sample( sNormal, input.uv ); // from texture; 
	float4 surface_emisive = tEmissive.Sample( sEmissive, input.uv ) * EMISSIVE_FACTOR;
	float4 normal_color = tNormal.Sample( sNormal, input.uv);
	float4 surface_ambient = tAmbient.Sample( sAmbient, input.uv );
	float4 surface_specular = tSpecular.Sample( sSpecular, input.uv );
